...
import { View, Text, ScrollView, ActivityIndicator, Image,Linking ,TouchableOpacity} from 'react-native';
...
class SunSafety extends Component {
  ...
  fetchSunSafety = () => {
    ...
    fetch('http://10.0.2.2/sunsafety', {
      ...
      .then((responseJson) => {
        this.setState({
          SunSafety: responseJson.tips,
          translatedSunSafety: responseJson.tips,
        }, () => {
          if (this.state.selectedLanguage !== 'en') {
            this.fetchTranslatedData(this.state.SunSafety, 'skincare');
          } else {
  ...
  fetchTranslatedData = (data, type) => {
    ...
    fetch('http://10.0.2.2:80/translate', {
      ...
      .then((response) => {
        ...
        if (type === 'skincare') {
          this.setState({ translatedSunSafety: translatedData.translated_text, loading: false });
        }
      })
      .catch((error) => {
        this.setState({ error: error.message, loading: false });
