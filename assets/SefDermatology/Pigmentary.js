...
class Pigmentary extends Component {
  ...
  fetchPigmentary = () => {
    ...
    fetch('http://10.0.2.2/pigmentary', {
      method: 'GET',
    })
      .then((response) => {
        if (!response.ok) {
      ...
      .then((responseJson) => {
        this.setState({
          Pigmentary: responseJson.tips,
          translatedPigmentary: responseJson.tips,
        }, () => {
          if (this.state.selectedLanguage !== 'en') {
            this.fetchTranslatedData(this.state.Pigmentary, 'skincare');
          } else {
  ...
  fetchTranslatedData = (data, type) => {
    ...
    fetch('http://10.0.2.2:80/translate', {
      ...
      .then((response) => {
        ...
        if (type === 'skincare') {
          this.setState({ translatedPigmentary: translatedData.translated_text, loading: false });
        }
      })
      .catch((error) => {
        this.setState({ error: error.message, loading: false });
