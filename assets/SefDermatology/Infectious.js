...
class Infectious extends Component {
  constructor(props) {
    ...
    this.subtitleImages = [
      ...
      // require('./banners/infectious/image16.png'),
      // require('./banners/infectious/image17.png'),
  ...
  fetchInfectious = () => {
    ...
    fetch('http://10.0.2.2/Infectious', {
      method: 'GET',
    })
      .then((response) => {
        if (!response.ok) {
      ...
      .then((responseJson) => {
        this.setState({
          Infectious: responseJson.tips,
          translatedInfectious: responseJson.tips,
        }, () => {
          if (this.state.selectedLanguage !== 'en') {
            this.fetchTranslatedData(this.state.Infectious, 'skincare');
          } else {
  ...
  fetchTranslatedData = (data, type) => {
    ...
    fetch('http://10.0.2.2:80/translate', {
      ...
      .then((response) => {
        ...
        if (type === 'skincare') {
          this.setState({ translatedInfectious: translatedData.translated_text, loading: false });
        }
      })
      .catch((error) => {
        this.setState({ error: error.message, loading: false });
