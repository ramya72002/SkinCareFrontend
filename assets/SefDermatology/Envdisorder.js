...
class Envdisorder extends Component {
  constructor(props) {
    ...
    this.subtitleImages = [
      ...
      // require('./banners/envdisorders/image32.png'),
  ...
  fetchEnvdisorder = () => {
    ...
    fetch('http://10.0.2.2/envdisorder', {
      method: 'GET',
    })
      .then((response) => {
        if (!response.ok) {
      ...
      .then((responseJson) => {
        this.setState({
          Envdisorder: responseJson.tips,
          translatedEnvdisorder: responseJson.tips,
        }, () => {
          if (this.state.selectedLanguage !== 'en') {
            this.fetchTranslatedData(this.state.Envdisorder, 'skincare');
          } else {
  ...
  fetchTranslatedData = (data, type) => {
    ...
    fetch('http://10.0.2.2:80/translate', {
      ...
      .then((response) => {
        ...
        if (type === 'skincare') {
          this.setState({ translatedEnvdisorder: translatedData.translated_text, loading: false });
        }
      })
      .catch((error) => {
        this.setState({ error: error.message, loading: false });
